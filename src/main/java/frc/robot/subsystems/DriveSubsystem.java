...
public class DriveSubsystem extends SubsystemBase {
    ...
    public DriveSubsystem() {
        ...
        rightMotorConfig.apply(globalConfig);
    ...
    public void simulationPeriodic() {
        ...
        m_leftMotorSim.iterate(
                (((kNEOMaxRPM * m_leftMotor.get()) / kDrivetrainGearRatio) * Math.PI * kWheelDiameterInches) / 60,
                RoboRioSim.getVInVoltage(), 0.02);
        m_rightMotorSim.iterate(
                (((kNEOMaxRPM * m_rightMotor.get()) / kDrivetrainGearRatio) * Math.PI * kWheelDiameterInches) / 60,
                RoboRioSim.getVInVoltage(), 0.02);

        m_leftMotorSim.setBusVoltage(RoboRioSim.getVInVoltage());
        m_rightMotorSim.setBusVoltage(RoboRioSim.getVInVoltage());

        m_leftMotorSim.setAppliedOutput(m_leftMotor.getAppliedOutput());
        m_rightMotorSim.setAppliedOutput(m_rightMotor.getAppliedOutput());
